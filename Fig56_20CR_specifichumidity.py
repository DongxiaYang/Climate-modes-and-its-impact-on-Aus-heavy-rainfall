...
for i in range(1950,2015,1):
    filelist.append(f'/g/data/ua8/LE_models/20CRv3/mean_daily/shum/shum.{i}.nc')
ds1=xr.open_mfdataset(filelist,combine='by_coords')
ds=ds1['shum'].loc[:,1000:500,-45:20,100:160]
vshu=ds.mean('level')
filename = "/g/data/w40/dy9345/MJO/"+'20cr_mean_shum_1k-500_elnino_1950-2014.nc'
vshu.to_netcdf(filename, 'w')
...
U_nina = data['shum'].loc[data.watch== 1]
phasenina= data['phase'].loc[data.watch== 1]
Ucli=U_nina.mean('time')
Ucli=Ucli.rename('shcli')

Uano=U_nina-Ucli
U_9 = Uano.groupby(phasenina).mean()
U_9=U_9.rename('shano')


outf=xr.merge([U_9,Ucli],join='inner').load()
filename = "/g/data/w40/dy9345/MJO/"+'20cr_mean_shumvmean_elnino_1950_2014.nc'
...
outf=xr.merge([U_9,Ucli],join='inner').load()
filename = "/g/data/w40/dy9345/MJO/"+'20cr_mean_shumvmean_lanina_1950_2014.nc'
outf.to_netcdf(filename, 'w')